...
import torch
from reamo.model import *
from reamo.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    if 'reamo' in model_name.lower():
        # Load LLaVA model
        if 'lora' in model_name.lower() and model_base is None:
            warnings.warn('There is `lora` in model name but no `model_base` is provided. If you are loading a LoRA model, please provide the `model_base` argument. Detailed instruction: https://github.com/haotian-liu/LLaVA#launch-a-model-worker-lora-weights-unmerged.')
        if 'lora' in model_name.lower() and model_base is not None:
            from reamo.model.language_model.reamo_llama import ReamoConfig
            base_cfg_pretrained = ReamoConfig.from_pretrained(model_base)
            print('Loading MUIE-Reamo from base model ...')
            model = ReamoLlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=base_cfg_pretrained, ignore_mismatched_sizes=True, **kwargs)
            
            print('Initialize MUIE-Reamo ...')
            tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)

            print('Loading additional MUIE-Reamo weights...')
            ...
            tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
            from reamo.model.language_model.reamo_llama import ReamoConfig
            cfg_pretrained = ReamoConfig.from_pretrained(model_base)
            print('cfg_pretrained: ', cfg_pretrained)
            model = ReamoLlamaForCausalLM.from_pretrained(model_base, config=cfg_pretrained).to(device="cuda", dtype=torch.float16) 
            print("kwargs: ", kwargs)
            ...
            model = ReamoLlamaForCausalLM.from_pretrained(
                model_path,
